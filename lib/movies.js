...
const TMDB_ACCESS_TOKEN = process.env.EXPO_PUBLIC_TMDB_ACCESS_TOKEN?.trim() || "";
...
const ensureTmdbConfigured = () => {
  if (!TMDB_API_KEY || !TMDB_ACCESS_TOKEN) {
    throw new Error("TMDB není nakonfigurováno. Zkontrolujte EXPO_PUBLIC_TMDB_API_KEY a EXPO_PUBLIC_TMDB_ACCESS_TOKEN.");
  }
};
...
export const getMovies = async () => {
  try {
    if (isSupabaseConfigured()) {
      if (!supabase) throw new Error("Supabase není dostupný.");
      ...
      return (data || []).map(normalizeMovieRow);
    }
...
export const addMovie = async (movie) => {
  try {
    ...
    if (isSupabaseConfigured()) {
      if (!supabase) return { success: false, error: "Supabase není dostupný." };
...
export const deleteAllMovies = async () => {
  try {
    ...
    console.error("deleteAllMovies error:", e);
    return { success: false, error: e?.message || "Error" };
  }
};
...
export const getMovieTrailerFromDb = async (movieDbId) => {
  try {
    if (isSupabaseConfigured()) {
      ...
      if (!error && data?.trailer_url) {
        return { url: data.trailer_url, site: "YouTube", name: "Trailer", key: null };
      }
      return null;
    }
    ...
    console.error("getMovieTrailerFromDb error:", e);
    return null;
  }
};
...
export const getMovieTrailer = async (movieId) => {
  ...
  if (t.site === "YouTube" && t.key) {
    return { key: t.key, name: t.name, site: t.site, url: `https://www.youtube.com/watch?v=${t.key}` };
  }
  if (t.site === "Vimeo" && t.key) {
    return { key: t.key, name: t.name, site: t.site, url: `https://vimeo.com/${t.key}` };
  }
